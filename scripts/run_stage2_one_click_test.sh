...
RUN_ALGORITHM="false"
BUILD_TYPE=""
# The production split-CFAR profile is applied only to the optional algorithm
# stage.  Pass --algorithm-xml-profile none for a legacy/template-only run.
ALGORITHM_XML_PROFILE="configs/eval/csi_split_repaired_overrides.json"
...
MOVING_TARGET_SINGLE_BEAM_ONLY=""
MOVING_TARGET_VISIBLE_BEAM_SPAN=""
...
Options:
  ...
  --moving-target-single-beam-only true|false
  --moving-target-visible-beam-span N
  ...
  --algorithm-xml-profile PATH
                             Default: configs/eval/csi_split_repaired_overrides.json
                             Use "none" to keep the generated XML unchanged
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --moving-target-single-beam-only) MOVING_TARGET_SINGLE_BEAM_ONLY="$2"; shift 2 ;;
    --moving-target-visible-beam-span) MOVING_TARGET_VISIBLE_BEAM_SPAN="$2"; shift 2 ;;
    ...
    --algorithm-xml-profile) ALGORITHM_XML_PROFILE="$2"; shift 2 ;;
...
if [[ -n "$MOVING_TARGET_SINGLE_BEAM_ONLY" ]]; then
  SIM_ARGS+=(--moving-target-single-beam-only "$MOVING_TARGET_SINGLE_BEAM_ONLY")
fi
if [[ -n "$MOVING_TARGET_VISIBLE_BEAM_SPAN" ]]; then
  SIM_ARGS+=(--moving-target-visible-beam-span "$MOVING_TARGET_VISIBLE_BEAM_SPAN")
fi
...
path = out_dir / "truth" / "truth_targets_by_beam.csv"
legacy_path = out_dir / "truth" / "target_truth_beam_summary.csv"
...
injected = [
    r for r in rows
    if to_float(r.get("injected_sample_count"), to_float(r.get("visible"), 0.0)) > 0.0
]
...
if [[ "$RUN_ALGORITHM" == "true" ]]; then
  ...
  ALGORITHM_XML="$OUTPUT_DIR/config/temp_config_stage2_newsystem.xml"
  if [[ "$ALGORITHM_XML_PROFILE" != "none" ]]; then
    if [[ ! -f "$ALGORITHM_XML_PROFILE" ]]; then
      echo "[oneclick][ERR] algorithm XML profile not found: $ALGORITHM_XML_PROFILE" >&2
      exit 1
    fi
    ALGORITHM_XML="$OUTPUT_DIR/config/effective_algorithm.xml"
    python3 scripts/configure_gmti_xml.py \
      --input "$OUTPUT_DIR/config/temp_config_stage2_newsystem.xml" \
      --output "$ALGORITHM_XML" \
      --set-json "$ALGORITHM_XML_PROFILE" \
      --set "result_add=$OUTPUT_DIR/algorithm_result" \
      --set "runtime_mode=debug" \
      --set "runtime_diagnostics_enabled=true" \
      --set "detection_results_csv_dump=true" \
      --set "track_debug_dump=true" \
      > "$OUTPUT_DIR/logs/configure_algorithm_xml.log"
    echo "[oneclick] algorithm XML profile: $ALGORITHM_XML_PROFILE"
  fi
  ./build/GMTI_core "$ALGORITHM_XML" \
