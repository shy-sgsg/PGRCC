...
def parse_args():
    ...
    parser.add_argument("--debug-dir", default="/home/shy/AIR/小长/GMTI_Data/Mission068/result_61beam/track_debug_runs/local_test_20260716_193306_534211_pid3234229")
    parser.add_argument("--coord", choices=["en", "geo"], default="geo")
    parser.add_argument("--out-dir", default="/home/shy/AIR/小长/GMTI_Data/Mission068/result_61beam/track_visualizations")
    ...
    parser.add_argument(
        ...
        default="latest",
        help=(
            "Independent TrackManager run to visualize: latest (default) or a "
            "non-negative index printed by the script. This prevents reused "
            "track_id values from separate process runs being connected."
        ),
    )
    ...
    parser.add_argument(
        "--make-gif", dest="make_gif", action="store_true",
        help="生成逐周期完整航迹增长 GIF（默认开启）",
    )
    parser.add_argument(
        "--no-gif", dest="make_gif", action="store_false",
        help="不生成逐周期完整航迹增长 GIF",
    )
    parser.add_argument(
        "--make-output-gif", dest="make_output_gif", action="store_true",
        help="生成逐周期协议输出航迹 GIF（默认开启）",
    )
    parser.add_argument(
        "--no-output-gif", dest="make_output_gif", action="store_false",
        help="不生成逐周期协议输出航迹 GIF",
    )
    parser.set_defaults(make_gif=True, make_output_gif=True)
    ...
    parser.add_argument(
        "--padding-ratio", type=float, default=0.08,
        help=(
            "空间图数据范围外侧留白比例；DBS image/warp 模式同样生效，"
            "避免背景图紧贴画布边框"
        ),
    )
    ...
    parser.add_argument("--dbs-image", default="/home/shy/AIR/小长/GMTI_Data/Mission068/result_61beam/GMTI13.png",
                        help="Path to DBS PNG image.")
    parser.add_argument("--dbs-corner-file", default="/home/shy/AIR/小长/GMTI_Data/Mission068/result_61beam/GMTI13.txt",
...
def _assign_snapshot_sequence(rows, frames, count_column, csv_name):
    """Map append-only detail rows back to their track_frames.csv snapshot."""
    out = rows.reset_index(drop=True).copy()
    out["_snapshot_seq"] = -1
    cursor = 0
    for snapshot_seq, frame in frames.iterrows():
        count = int(frame[count_column])
        if count < 0:
            raise ValueError(
                f"track_frames.csv has negative {count_column} at row {snapshot_seq}"
            )
        end = cursor + count
        if end > len(out):
            raise ValueError(
                f"{csv_name} is truncated: frame row {snapshot_seq} expects "
                f"{count} rows, only {len(out) - cursor} remain"
            )
        if count:
            chunk = out.iloc[cursor:end]
            expected_result_id = frame["result_id"]
            if not chunk["result_id"].eq(expected_result_id).all():
                actual_ids = sorted(chunk["result_id"].dropna().unique().tolist())
                raise ValueError(
                    f"{csv_name} does not align with track_frames.csv at frame row "
                    f"{snapshot_seq}: expected result_id={expected_result_id}, "
                    f"got {actual_ids}"
                )
            out.loc[cursor:end - 1, "_snapshot_seq"] = snapshot_seq
        cursor = end
    if cursor != len(out):
        raise ValueError(
            f"{csv_name} has {len(out) - cursor} trailing rows not described by "
            "track_frames.csv"
        )
    return out


def _track_manager_restarted(previous_states, current_states):
    """Detect ID reuse/age rollback that cannot occur in one TrackManager."""
    if previous_states.empty or current_states.empty:
        return False
    if "age" not in previous_states.columns or "age" not in current_states.columns:
        return False
    overlap = previous_states[["track_id", "age"]].merge(
        current_states[["track_id", "age"]],
        on="track_id",
        how="inner",
    ...
    if overlap.empty:
        return False
    return bool((overlap["age_current"] <= overlap["age_previous"]).any())


def annotate_debug_runs(frames, detections, states):
    """Recover independent TrackManager runs from append-only legacy CSVs.

    The historical CSV schema has no process/run identifier.  The frame file is
    nevertheless authoritative for row counts, so detail rows can first be
    mapped to snapshot occurrences.  A non-increasing result_id, an impossible
    track-age rollback, or an empty manager immediately after a non-empty one
    marks a new run.  Empty-only runs are ignored when assigning public indices.
    """
    frames_out = frames.reset_index(drop=True).copy()
    frames_out["_snapshot_seq"] = np.arange(len(frames_out), dtype=np.int64)
    detections_out = _assign_snapshot_sequence(
        detections, frames_out, "num_detections", "track_detections.csv"
    )
    states_out = _assign_snapshot_sequence(
        states, frames_out, "num_tracks", "track_states.csv"
    )
    ...
    for snapshot_seq, frame in frames_out.iterrows():
        current_states = states_out[states_out["_snapshot_seq"] == snapshot_seq]
        if previous_frame is not None:
            result_id_not_increasing = frame["result_id"] <= previous_frame["result_id"]
            became_empty = (
                int(previous_frame["num_tracks"]) > 0
                and int(frame["num_tracks"]) == 0
            )
            age_rolled_back = _track_manager_restarted(
                previous_states, current_states
            )
            if result_id_not_increasing or became_empty or age_rolled_back:
                raw_run += 1
        raw_run_ids.append(raw_run)
        previous_frame = frame
        previous_states = current_states

    frames_out["_raw_run_index"] = raw_run_ids
    raw_run_for_snapshot = dict(
        zip(frames_out["_snapshot_seq"], frames_out["_raw_run_index"])
    )
    detections_out["_raw_run_index"] = detections_out["_snapshot_seq"].map(
        raw_run_for_snapshot
    )
    states_out["_raw_run_index"] = states_out["_snapshot_seq"].map(
        raw_run_for_snapshot
    )

    nonempty_raw_runs = sorted(states_out["_raw_run_index"].unique().tolist())
    dense_run_index = {raw: dense for dense, raw in enumerate(nonempty_raw_runs)}
    frames_out["_run_index"] = (
        frames_out["_raw_run_index"].map(dense_run_index).fillna(-1).astype(int)
    )
    detections_out["_run_index"] = (
        detections_out["_raw_run_index"].map(dense_run_index).astype(int)
    )
    states_out["_run_index"] = (
        states_out["_raw_run_index"].map(dense_run_index).astype(int)
    )
    return frames_out, detections_out, states_out


def select_debug_run(frames, detections, states, requested):
    available = sorted(states["_run_index"].unique().tolist())
    if not available:
        raise ValueError("no non-empty TrackManager runs found in debug CSVs")

    summaries = []
    for run_index in available:
        run_frames = frames[frames["_run_index"] == run_index]
        result_ids = run_frames["result_id"].astype(int).tolist()
        summaries.append(f"{run_index}: result_id={result_ids}")
    print("Detected TrackManager runs: " + "; ".join(summaries))

    if requested == "latest":
        selected = available[-1]
        if len(available) > 1:
            print(
                f"Multiple runs share this debug directory; selecting latest run "
                f"{selected}. Use --run-index N to inspect another run."
            )
    else:
        try:
            selected = int(requested)
        except (TypeError, ValueError) as exc:
            raise ValueError("--run-index must be 'latest' or a non-negative integer") from exc
        if selected < 0 or selected not in available:
            raise ValueError(
                f"--run-index {selected} is unavailable; choose one of {available}"
            )

    return (
        frames[frames["_run_index"] == selected].copy(),
        detections[detections["_run_index"] == selected].copy(),
        states[states["_run_index"] == selected].copy(),
        selected,
    )
...
def compute_axis_limits_with_dbs(states, detections, roi, coord, padding_ratio, dbs_bg, axis_from):
    if dbs_bg is not None and dbs_bg.get("mode") == "image":
        xmax = float(dbs_bg["width"] - 1)
        ymax = float(dbs_bg["height"] - 1)
        xpad = max(1.0, xmax * max(0.0, padding_ratio))
        ypad = max(1.0, ymax * max(0.0, padding_ratio))
        return (-xpad, xmax + xpad), (ymax + ypad, -ypad)
    if dbs_bg is not None and dbs_bg.get("mode") == "warp":
        extent = dbs_bg["extent"]
        return (
            padded_limits([extent[0], extent[1]], padding_ratio=padding_ratio),
            padded_limits([extent[2], extent[3]], padding_ratio=padding_ratio),
        )
...
def main():
    ...
    try:
        ...
        frames, detections, states = annotate_debug_runs(
            frames, detections, states
        )
        frames, detections, states, selected_run = select_debug_run(
            frames, detections, states, args.run_index
        )
        print(f"Visualizing TrackManager run {selected_run}")
