...
struct P38StageMetrics {
    ...
    double median_abs_residual = std::numeric_limits<double>::quiet_NaN();
    double p90_abs_residual = std::numeric_limits<double>::quiet_NaN();
    double p95_abs_residual = std::numeric_limits<double>::quiet_NaN();
    double max_abs_residual = std::numeric_limits<double>::quiet_NaN();
    int sample_count = 0;
    double inlier_ratio = std::numeric_limits<double>::quiet_NaN();
    int strong_range_masked_bins = 0;
    bool valid = false;
    std::string source;
};
                                             ...
                                             const std::array<double, 2> &p38)
{
    ...
    absolute_residuals.reserve(phase_samples.size());
    ...
    if (valid > 0) {
        ...
        const auto quantile = [&absolute_residuals](double probability) {
            const double scaled = probability *
                static_cast<double>(absolute_residuals.size() - 1U);
        ...
        out.median_abs_residual = quantile(0.50);
        out.p90_abs_residual = quantile(0.90);
        out.p95_abs_residual = quantile(0.95);
        out.max_abs_residual = absolute_residuals.back();
...
// Local CFAR/cluster guards do not cover the wide coherent range sidelobes of
// a clipped or very strong LFM echo.  Detect exceptional range gates from the
// same pre-cancellation dual-channel data used by refit, dilate in range, and
// exclude each affected gate over the full aperture.  This 10x-median / +/-32
// rule matches the CUDA startup fallback and the independent p38 audit script.
template <typename ComplexT>
inline int addStrongRangeOutlierMask(const std::vector<ComplexT> &F1,
                                     const std::vector<ComplexT> &F2,
                                     int Na,
                                     int Nr,
                                     int az_st,
                                     int rg_st,
                                     ...
                                     double strong_ratio = 10.0,
                                     int dilation_bins = 32)
{
    const size_t total = static_cast<size_t>(std::max(0, Na)) *
                         static_cast<size_t>(std::max(0, Nr));
    if (Na <= 0 || Nr <= 0 || F1.size() != total || F2.size() != total ||
        mask.size() != total || !(strong_ratio > 0.0)) {
        return 0;
    }
    az_st = std::max(0, std::min(az_st, Na - 1));
    az_ed = std::max(0, std::min(az_ed, Na - 1));
    rg_st = std::max(0, std::min(rg_st, Nr - 1));
    rg_ed = std::max(0, std::min(rg_ed, Nr - 1));
    if (az_st > az_ed || rg_st > rg_ed) return 0;

    std::vector<double> magnitude(static_cast<size_t>(Nr), 0.0);
    std::vector<double> finite_positive;
    finite_positive.reserve(static_cast<size_t>(rg_ed - rg_st + 1));
    for (int c = rg_st; c <= rg_ed; ++c) {
        std::complex<double> sum(0.0, 0.0);
        for (int r = az_st; r <= az_ed; ++r) {
            const size_t idx = static_cast<size_t>(r) * static_cast<size_t>(Nr) +
                               static_cast<size_t>(c);
            const std::complex<double> a(static_cast<double>(F1[idx].real()),
                                         static_cast<double>(F1[idx].imag()));
            const std::complex<double> b(static_cast<double>(F2[idx].real()),
                                         static_cast<double>(F2[idx].imag()));
        ...
        magnitude[static_cast<size_t>(c)] = value;
        if (std::isfinite(value) && value > 0.0) finite_positive.push_back(value);
    }
    if (finite_positive.size() < 8U) return 0;
    std::sort(finite_positive.begin(), finite_positive.end());
    const size_t n = finite_positive.size();
    const double median = (n % 2U == 0U)
        ? 0.5 * (finite_positive[n / 2U - 1U] + finite_positive[n / 2U])
        : finite_positive[n / 2U];
    if (!(std::isfinite(median) && median > 0.0)) return 0;
    const double threshold = strong_ratio * median;

    std::vector<uint8_t> range_mask(static_cast<size_t>(Nr), 0U);
    const int dilation = std::max(0, dilation_bins);
    for (int c = rg_st; c <= rg_ed; ++c) {
        const double value = magnitude[static_cast<size_t>(c)];
        if (!(std::isfinite(value) && value > threshold)) continue;
        const int lo = std::max(rg_st, c - dilation);
        const int hi = std::min(rg_ed, c + dilation);
        for (int j = lo; j <= hi; ++j) range_mask[static_cast<size_t>(j)] = 1U;
    }
    const int masked_ranges = static_cast<int>(std::count(
        range_mask.begin() + rg_st, range_mask.begin() + rg_ed + 1,
        static_cast<uint8_t>(1U)));
    const int support_ranges = rg_ed - rg_st + 1;
    if (masked_ranges <= 0 || masked_ranges + 8 >= support_ranges) return 0;

    for (int r = 0; r < Na; ++r) {
        const size_t off = static_cast<size_t>(r) * static_cast<size_t>(Nr);
        for (int c = rg_st; c <= rg_ed; ++c) {
            if (range_mask[static_cast<size_t>(c)] != 0U) {
                mask[off + static_cast<size_t>(c)] = 1U;
            }
        }
    }
                                        ...
                                        std::vector<uint8_t> *mask_out = nullptr)
{
    ...
    const int strong_range_masked_bins = addStrongRangeOutlierMask(
        F1_pre, F2_pre, Na, Nr, az_st, rg_st, az_ed, rg_ed, mask);
    if (mask_out) {
        *mask_out = mask;
    }
    ...
    metrics.strong_range_masked_bins = strong_range_masked_bins;
    return metrics;
}
