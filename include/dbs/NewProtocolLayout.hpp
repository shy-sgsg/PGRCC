...
static const std::size_t kOffHeadingDeg = 160;
static const std::size_t kOffPrtLowByte = 208;
static const std::size_t kOffServoAzimuthDegX100 = 218;
static const std::size_t kOffServoElevationDegX100 = 220;
static const std::size_t kOffScanCenterAzimuthDegX100 = 222;
static const std::size_t kOffScanCenterElevationDegX100 = 224;
static const std::size_t kOffScanSpeedAzDegPerSecX2 = 226;
static const std::size_t kOffScanExtentAzDegX100 = 227;
// Compatibility name used by existing electronic-scan writers/readers.  The
// protocol field is the measured relative servo azimuth, not a derived beam id.
static const std::size_t kOffThetaDegX100 = kOffServoAzimuthDegX100;
...
inline int16_t satI16FromDouble(double x)
{
...
{
    return satI16FromDouble(static_cast<double>(x));
}
...
struct HeaderSample {
    ...
    // Kept for source compatibility. It is identical to servo_azimuth_deg.
...
inline HeaderSample readHeaderSample(const uint8_t *hdr)
{
    ...
    s.heading_deg = loadF64LE(hdr + kOffHeadingDeg);
    s.prt_counter = loadU32LE(hdr + kOffPrtCounter);
    s.servo_azimuth_deg =
        static_cast<double>(loadI16LE(hdr + kOffServoAzimuthDegX100)) / 100.0;
    s.servo_elevation_deg =
        static_cast<double>(loadI16LE(hdr + kOffServoElevationDegX100)) / 100.0;
    s.scan_center_azimuth_deg =
        static_cast<double>(loadI16LE(hdr + kOffScanCenterAzimuthDegX100)) / 100.0;
    s.scan_center_elevation_deg =
        static_cast<double>(loadI16LE(hdr + kOffScanCenterElevationDegX100)) / 100.0;
    s.scan_speed_az_deg_s =
        static_cast<double>(hdr[kOffScanSpeedAzDegPerSecX2]) * 0.5;
    s.scan_extent_az_deg =
        static_cast<double>(loadI16LE(hdr + kOffScanExtentAzDegX100)) / 100.0;
    s.theta_cmd_deg = s.servo_azimuth_deg;
    return s;
}
