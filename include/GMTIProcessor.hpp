...
public:
                      ...
                      cached_Na_(0), cached_Nr_(0), stream_compute_(0)
    {
        ...
        // 初始化指针结构体为 null
        gpu_ptrs_ = { nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr };
    }
    ...
    // 根据 cfg 选择策略，生成 beamList
    bool makeBeamList(const GMTIOutput::Plane& plane,
                      const Config& cfg,
                      std::vector<int>& beamList);
    bool makeMechanicalCpiList(const Config& cfg,
                               std::vector<int>& windowList);
    ...
    // 并行处理多个 beam（在单 GPU 上使用多个 GMTIProcessor 实例并发运行）
    // 输出 results 与输入 beamList 顺序一致
    bool processBeamsParallel(const std::vector<int> &beamList,
                              const Config &cfg,
                              const std::vector<std::vector<double>> &posRaw,
                              std::vector<GMTIOutput> &results);
    bool processBeamsParallelFusion(const std::vector<int> &beamList,
                                    const Config &cfg,
                                    const std::vector<std::vector<double>> &posRaw,
                                    FusionGroupContext &ctx);
    bool processBeamsParallelFusion(const std::vector<int> &beamList,
                                    ...
                                    FusionGroupContext &ctx,
                                    std::vector<GMTIOutput> &results);
    // 在实时循环开始前完成 DBS 融合 worker 的 CUDA/FFT 资源创建，使
    // 首个有效采集周期与后续周期采用同一资源状态。
    bool prewarmFusionWorkers(const Config &cfg);
    ...
    bool computeDatasetSquintFromCenter(const std::vector<int> &beamList,
                                        const Config &cfg,
                                        const std::vector<std::vector<double>> &posRaw,
                                        double &out_squint);
...
private:
    ...
    bool pulseCompressionGpuResident(const std::vector<std::complex<float>> &data1,
                                     const std::vector<std::complex<float>> &data2,
                                     const Config &cfg);
    bool pulseCompressionGpuResident(const NewProtocolGpuInput &input,
                                     const Config &cfg);
    ...
    bool clutter_cancel_38_paper_1(
        ...
        std::string *fit_model_source = nullptr);
    ...
    bool dpca_cfar2_fast_cuda(const std::vector<std::complex<float>> &CSI_out,
                              ...
                              std::vector<float> *power_map = nullptr,
                              std::vector<std::complex<float>> *detect_map = nullptr,
    ...
    bool cluster_filter_gap_phase_cuda(const std::vector<float> &mydata,
                                       ...
                                       bool download_refined_data = true,
                                       bool use_resident_cfar = true,
                                       double *global_median_power = nullptr,
                                       std::size_t *baseline_kept = nullptr,
                                       std::size_t *strong_small_kept = nullptr,
                                       std::size_t *small_rejected = nullptr,
                                       bool *strong_filter_applied = nullptr);
    ...
    size_t gpu_single_buffer_bytes_ = 0; // 主缓冲区跨度，供原始 payload 暂存边界检查
    ...
    struct {
        ...
        void* dbs_amplitude; // DBS幅度导出专用缓冲（不得复用复数数据缓冲）
    } gpu_ptrs_;

    size_t dbs_amplitude_elements_ = 0;

    float* d_phi_fit_ = nullptr; // 距离向相位校正的拟合值（长度 cfg.rg_len）

    // CFAR/聚类临时区：每个并行worker持有并跨波位复用，避免每个波位
    // 反复cudaMalloc/cudaFree造成设备级隐式同步。两个阶段顺序执行，共用一块区域。
    void* d_detection_workspace_ = nullptr;
    size_t d_detection_workspace_bytes_ = 0;

    // CFAR 命中图和功率图跨 CFAR/聚类阶段驻留。与通用检测 workspace
    // 分开分配，聚类扩容 workspace 时不会使上一阶段输出失效。
    float* d_cfar_maps_ = nullptr;
    size_t d_cfar_map_elements_ = 0;
    bool d_cfar_maps_valid_ = false;

    // CSI 前相位快照：在最终 CSI/CFAR 覆盖 t1/t2 前保留在设备端。常规
    // GPU 聚类直接消费该快照；只有 P38 二次拟合、CPU 回退或最终候选点需要
    // 时才回传，避免每个波位一次完整 D2H 再 H2D 往返。
    float* d_phase_snapshot_ = nullptr;
    size_t d_phase_snapshot_elements_ = 0;

    // 固定布局的逐波位临时区。它们原先在每次 P38/CSI 调用中反复
    // cudaMalloc/cudaFree；按 worker 持久复用可避免设备级隐式同步。
    float* d_p38_phase_ = nullptr;
    size_t d_p38_phase_elements_ = 0;
    float* d_csi_fa_ = nullptr;
    size_t d_csi_fa_elements_ = 0;
    cuFloatComplex* d_csi_alpha_ = nullptr;
    size_t d_csi_alpha_elements_ = 0;
    float* d_csi_coherence_ = nullptr;
    size_t d_csi_coherence_elements_ = 0;
    ...
    // DBS 融合路径在相同数据布局的连续扫描间复用 worker。每个 worker
    // 持有独立 stream、FFT plan 和检测 workspace，避免逐周期反复创建/销毁
    // CUDA 资源引入的全局同步和长尾抖动。
    std::vector<std::unique_ptr<GMTIProcessor>> fusion_workers_;
    bool fusion_worker_layout_valid_ = false;
    int fusion_worker_proc_pulse_num_ = 0;
    int fusion_worker_pulse_dec_ = 0;
    int fusion_worker_rg_len_ = 0;
    int fusion_worker_range_fft_len_ = 0;
    ...
    bool cuda_upload_async(const std::vector<std::complex<float>> &data1,
                           const std::vector<std::complex<float>> &data2,
                           size_t Na, size_t Nr);
    bool cuda_decode_new_protocol_async(const NewProtocolGpuInput &input,
                                        const Config &cfg);

    // --- 第二阶段：通道对齐 Kernel ---
    bool cuda_stage_align_async(int skip, size_t Na, size_t Nr, const Config& cfg);
    ...
    bool exportDbsCacheAfterRecenter(const Config& cfg,
                                     ...
                                     RDData& rd,
                                     MetaPack& meta);
    bool cuda_export_dbs_amplitude_sync(std::vector<float> &amplitude,
                                        bool &has_signal,
                                        size_t total);
    ...
    bool cuda_capture_phase_map_async(size_t total);
    bool cuda_download_phase_map(std::vector<float> &phase_map, size_t total);
    bool cuda_download_phase_samples(const std::vector<int> &prow,
                                     const std::vector<int> &pcol,
                                     int width,
                                     std::vector<float> &phase_samples);
    bool cuda_download_cfar_maps(std::vector<float> &hits,
                                 std::vector<float> &power,
                                 size_t total);
    bool cuda_download_cfar_power_samples(const std::vector<int> &prow,
                                          const std::vector<int> &pcol,
                                          int width,
                                          std::vector<float> &power_samples);
    bool cuda_download_csi_sync(std::vector<std::complex<float>> &out, size_t total);
    bool ensureP38PhaseWorkspace(size_t elements);
    bool ensureCsiFrequencyWorkspace(size_t elements);
    bool ensureCsiRowWorkspace(size_t elements);
    bool ensureDetectionWorkspace(size_t needed_bytes);
    bool ensureCfarMaps(size_t total);
    bool ensureFusionWorkers(size_t required_workers, const Config &cfg);
    size_t fusionWorkerCountForMemory(const Config &cfg,
                                      size_t requested_workers,
                                      size_t free_bytes) const;
    bool fusionWorkerLayoutMatches(const Config &cfg) const;
    void invalidateFusionWorkers();
