...
> 研究主线更新（2026-09-13）：本文保留历史输入、方法和数字。“两组结果按信息条件分开汇报”不等于四通道 STAP 不能与两通道 Current CSI 做端到端比较。当前要求同时报告端到端能力层和信息量匹配层；本文中的 Oracle 默认解释为已知误差校正上限。
