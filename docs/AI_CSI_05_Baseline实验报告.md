...
> 研究主线更新（2026-09-13）：本报告数字和原始比较口径保持不变。四通道 academic reference 未混入严格 F1/F2 排名，是信息条件审计，不是禁止它与两通道 Current CSI 做端到端比较。后续正式比较需增加端到端能力层与信息量匹配层；文中的 Oracle 按已知误差校正上限理解。
...
本轮没有训练 AI，也没有端到端 `clutter-free RD` 网络。按当前主线，下一步先做真实
系统未知误差的观测、确定性估计和物理校准；只有稳定且难以解析的残差仍存在时，才
重新评估 Physics-AI。
