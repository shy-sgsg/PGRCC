...
// The periodic tables contain samples at [0, 2*pi).  Use the bin floor and
// interpolate to the next sample; wrapping the upper index makes the interval
// around 2*pi continuous instead of selecting one nearest-neighbour sample.
inline PeriodicLutCoord periodicLutCoord(double x)
{
    const double scaled = x * (static_cast<double>(kSize) / kTwoPi);
    const double lower_real = std::floor(scaled);
    const long long lower_unwrapped = static_cast<long long>(lower_real);
    long long lower_wrapped = lower_unwrapped % static_cast<long long>(kSize);
    if (lower_wrapped < 0) lower_wrapped += static_cast<long long>(kSize);
    PeriodicLutCoord coord;
    coord.lower = static_cast<int>(lower_wrapped);
    coord.upper = (coord.lower + 1) & kMask;
    coord.fraction = scaled - lower_real;
    return coord;
}

inline double interpolatePeriodic(const std::array<float, kSize>& table,
                                   const PeriodicLutCoord& coord)
{
    const double y0 = static_cast<double>(table[coord.lower]);
    const double y1 = static_cast<double>(table[coord.upper]);
    return y0 + coord.fraction * (y1 - y0);
}

inline double interpolateUnit(const std::array<float, kSize + 1>& table,
                               double x, double lower, double upper)
{
    x = std::max(lower, std::min(upper, x));
    const double scaled = (x - lower) / (upper - lower) *
                          static_cast<double>(kSize);
    const int index = static_cast<int>(std::floor(scaled));
    if (index >= kSize) return static_cast<double>(table[kSize]);
    const double fraction = scaled - static_cast<double>(index);
    const double y0 = static_cast<double>(table[index]);
    const double y1 = static_cast<double>(table[index + 1]);
    return y0 + fraction * (y1 - y0);
}

inline double interpolateZeroOne(const std::array<float, kSize + 1>& table,
                                 double x)
{
    return interpolateUnit(table, x, 0.0, 1.0);
}

inline double lutSin(double x)
{
    return interpolatePeriodic(g_sin_table, periodicLutCoord(x));
}

inline double lutCos(double x)
{
    return interpolatePeriodic(g_cos_table, periodicLutCoord(x));
}

inline double lutTan(double x)
{
    const PeriodicLutCoord coord = periodicLutCoord(x);
    const double sine = interpolatePeriodic(g_sin_table, coord);
    const double cosine = interpolatePeriodic(g_cos_table, coord);
    return sine / cosine;
}

inline double lutAsin(double x)
{
    return interpolateUnit(g_asin_table, x, -1.0, 1.0);
}
...
{
    if (x > 1.0) {
        return 0.5 * kPi - interpolateZeroOne(g_atan_table, 1.0 / x);
    }
    if (x < -1.0) {
        return -0.5 * kPi + interpolateZeroOne(g_atan_table, -1.0 / x);
    }
    const double ax = std::fabs(x);
    const double a = interpolateZeroOne(g_atan_table, ax);
    return x < 0.0 ? -a : a;
}
