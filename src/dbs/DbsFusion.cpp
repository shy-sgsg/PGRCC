...
bool fusionSlotHasSignal(const FusionGroupContext &ctx, size_t slot)
{
    ...
    if (slot < ctx.rd.has_signal.size()) {
        return ctx.rd.has_signal[slot] != 0;
    }
                                           ...
                                           double &biasDeg)
{
    ...
    if (!beamMeta.empty() &&
        beamMeta.front().scan_mode == ScanMode::Mechanical) {
        return estimatePointingBiasFromMechanicalWindows(beamMeta, biasDeg);
    }
...
bool estimatePointingBiasFromMechanicalWindows(
    const std::vector<FusionBeamMeta> &windowMeta,
    double &biasDeg)
{
    biasDeg = 0.0;
    if (windowMeta.empty()) return false;
    double az_min = std::numeric_limits<double>::infinity();
    double az_max = -std::numeric_limits<double>::infinity();
    for (const FusionBeamMeta& m : windowMeta) {
        if (!std::isfinite(m.az_center_deg)) continue;
        az_min = std::min(az_min, m.az_center_deg);
        az_max = std::max(az_max, m.az_center_deg);
    }
    if (!std::isfinite(az_min) || !std::isfinite(az_max)) return false;
    const double scan_center = 0.5 * (az_min + az_max);
    std::vector<std::pair<double, std::size_t>> ranked;
    for (std::size_t i = 0; i < windowMeta.size(); ++i) {
        if (std::isfinite(windowMeta[i].az_center_deg)) {
            ranked.push_back(std::make_pair(
                std::abs(windowMeta[i].az_center_deg - scan_center), i));
        }
    }
    ...
    for (std::size_t i = 0; i < take; ++i) {
        const FusionBeamMeta& m = windowMeta[ranked[i].second];
        double estimated_angle = 0.0;
        if (!estimate_angle_from_fd(m, estimated_angle)) continue;
        sum += estimated_angle - m.theta_sq;
        ++count;
    }
    if (count == 0) return false;
    biasDeg = sum / static_cast<double>(count);
    return std::isfinite(biasDeg);
}
    ...
    for (size_t slot = 0; slot < n; ++slot) {
        ...
        for (const DetectionRaw &d : rawList) {
            ...
            const double motionAfTotal = cfg.motion_comp_use_row_doppler
                ? (d.af_total + faShift)
                : legacyAf;
            double calibratedRangePhaseCorrection = d.range_phase_correction;
            if (cfg.channel_calibration_enable &&
                cfg.channel_calibration_apply_to_localization &&
                m.channel_calibration_valid &&
                std::isfinite(calibratedRangePhaseCorrection)) {
                calibratedRangePhaseCorrection -=
                    m.channel_calibration_phase_bias_rad;
            }
            MotionCompResult motion_pre =
                solveMotionCompensation(cfg, m.plane, d.phase, motionAfTotal,
                                        m.p38_pre_k, m.p38_pre_b, lambda,
                                        d.range_m, m.theta_true,
                                        calibratedRangePhaseCorrection);
            MotionCompResult motion_refit = motion_pre;
            if (cfg.p38_enhanced_enable && m.p38_used_source == "refit") {
                motion_refit = solveMotionCompensation(
                    cfg, m.plane, d.phase, motionAfTotal,
                    m.p38_refit_k, m.p38_refit_b, lambda,
                    d.range_m, m.theta_true,
                    calibratedRangePhaseCorrection);
            }
            MotionCompResult motion =
                (cfg.p38_enhanced_enable && m.p38_used_source == "refit")
            ...
            // Keep the legacy single-period velocity compensation available for
            // diagnostics, but do not let it replace p38/CTDR localization unless
            // the configuration explicitly opts in.
            bool useCompLoc = cfg.motion_comp_enable &&
                              cfg.motion_comp_apply_to_localization &&
                              motion.ok && compValid;
            double af = useCompLoc ? motion.af_geometry : legacyAf;
            double sinA = useCompLoc ? sinAComp : sinAOld;
            const char *locUsedMode = useCompLoc
            ...
            double dE = xP - m.plane.E;
            double dN = yP - m.plane.N;
            double targetAzimuthDeg = normalize_azimuth_deg(gmti::trig_lut::atan2(dN, dE) * 180.0 / M_PI);
            double direction = wrap180_deg(m.plane.V_angle - targetAzimuthDeg);
            ...
            double beamDirErr = wrap180_deg(direction - beamCenterDir);
            if (std::abs(beamDirErr) > beamHalfWidth && useCompLoc && oldValid &&
                std::isfinite(oldXP) && std::isfinite(oldYP)) {
                const double oldDE = oldXP - m.plane.E;
                const double oldDN = oldYP - m.plane.N;
                const double oldAz = normalize_azimuth_deg(
                    gmti::trig_lut::atan2(oldDN, oldDE) * 180.0 / M_PI);
                const double oldDirection = wrap180_deg(m.plane.V_angle - oldAz);
                const double oldBeamErr = wrap180_deg(oldDirection - beamCenterDir);
                if (std::abs(oldBeamErr) <= beamHalfWidth) {
                    useCompLoc = false;
                    af = legacyAf;
                    sinA = sinAOld;
                    locUsedMode = "p38_beam_fallback";
                    motionCompStatus = "ctdr_outside_beam_fallback_p38";
                    thetaUsedDeg = thetaOldUsedDeg;
                    lookFromSinAE = lookOldE;
                    lookFromSinAN = lookOldN;
                    xP = oldXP;
                    yP = oldYP;
                    dE = oldDE;
                    dN = oldDN;
                    targetAzimuthDeg = oldAz;
                    direction = oldDirection;
                    beamDirErr = oldBeamErr;
                    (void)Gaussp3RV(xP, yP, cfg.L0, lat, lng);
                }
            }
            ...
            rec.period_id = cfg.echo_cycle_view.valid()
                ...
                : ((cfg.INFO_Type && !cfg.GMTI_Data_new.empty())
                       ...
                       : ((cfg.result_file_id > 0) ? (cfg.result_file_id - 1) : 0)));
            rec.beam_id = m.scan_mode == ScanMode::Mechanical ? -1 : m.beam_index;
            rec.scan_mode = m.scan_mode;
            rec.scan_id = m.scan_id;
            rec.window_id = m.window_id;
            rec.pulse_start = m.pulse_start;
            rec.pulse_end = m.pulse_count == 0U
                ? m.pulse_start : m.pulse_start + m.pulse_count - 1U;
            rec.cpi_center_utc = m.utc_mid;
            rec.az_start_deg = m.az_start_deg;
            rec.az_center_deg = m.az_center_deg;
            rec.az_end_deg = m.az_end_deg;
            rec.az_span_deg = m.az_span_deg;
            rec.scan_direction = m.scan_direction;
...
bool runDbsFusionImaging(FusionGroupContext &ctx,
                         const Config &cfg,
                         bool useGpu)
{
    ...
    for (size_t slot = 0; slot < ctx.meta.beams.size(); ++slot) {
        ...
        // DBS 成像是该周期 RD 缓存的最后一个消费者。转移所有权可避免
        // 在 CPU 上将整个 59 波位 128x4096 float 幅度缓存（118 MiB）再深拷贝一次。
        activeCtx.rd.amp.push_back(std::move(ctx.rd.amp[slot]));
        activeCtx.rd.fd_axis.push_back(std::move(ctx.rd.fd_axis[slot]));
        activeCtx.rd.rg_axis.push_back(std::move(ctx.rd.rg_axis[slot]));
        activeCtx.rd.has_signal.push_back(1);
