...
bool GMTIProcessor::makeBeamList(const GMTIOutput::Plane& plane,
                                 const Config& cfg,
                                 std::vector<int>& beamList)
{
    if (!cfg.wavepos_use_roi) {
        return makeDefaultBeamList(cfg, beamList);
    } else {
        return makeBeamListROI51(cfg, plane, beamList);
    }
}

bool GMTIProcessor::makeMechanicalCpiList(const Config& cfg,
                                          std::vector<int>& windowList)
{
    ...
    windowList = cfg.mechanical_scan_runtime->processing_window_indices;
    return !windowList.empty();
}

bool GMTIProcessor::makeBeamListROI51(const Config& cfg,
                                      ...
                                      std::vector<int>& beamList)
{
    ...
    for (size_t i = begin; i <= end; ++i) {
        beamList.push_back(candidates[i]);
    }

    return true;
}

bool GMTIProcessor::makeDefaultBeamList(const Config& cfg, std::vector<int>& beamList)
{
    beamList.clear();
    ...
    for (int v = st; v <= ed; v += step) beamList.push_back(v);
