...
bool GMTIProcessor::rangeCompressCUFFT_device(int Lraw, int M, const Config &cfg) {
    ...
    std::vector<double> spectral_weights;
    try {
        spectral_weights = gmti::range_compression::buildSpectralWeights(cfg, Nfft);
    ...
    for (int n=0;n<Nfft;++n) {
        ...
        const float weight = static_cast<float>(spectral_weights[static_cast<std::size_t>(n)]);
        h_Hf[n] = make_cuFloatComplex(re * weight, im * weight);
    }
...
bool GMTIProcessor::rangeCompressCUFFT_device_pair(int Lraw, int M, const Config &cfg) {
    ...
    for (int n = 0; n < Nfft; ++n) {
        ...
        double phase = M_PI * (fn * fn) / Kr;
        const float weight = static_cast<float>(spectral_weights[static_cast<std::size_t>(n)]);
        h_Hf[n] = make_cuFloatComplex(
            (float)gmti::trig_lut::cos(phase) * weight,
            (float)gmti::trig_lut::sin(phase) * weight);
    }
