                                    ...
                                    std::vector<float> *power_map)
{
    ...
    {
        std::cerr << "dpca_cfar2: circular Doppler CFAR window exceeds row count\n";
        return false;
    }
    ...
    auto doppler_rect_sum = [&](int r0, int c0, int r1, int c1) -> double
    {
        if (!cfg.cfar_doppler_circular || (r0 >= 0 && r1 < H))
            return rect_sum(r0, c0, r1, c1);
        if (r0 < 0)
            return rect_sum(0, c0, r1, c1) +
                   rect_sum(H + r0, c0, H - 1, c1);
        return rect_sum(r0, c0, H - 1, c1) +
               rect_sum(0, c0, r1 - H, c1);
    };

    // —— 2) 预计算 alpha
    const double alpha = ty == "GO"
        ? gmti::go_cfar::calibrated_alpha(pf, g, b)
        : double(total_bg) * (std::pow(pf, -1.0 / double(total_bg)) - 1.0);

    // —— 3) 遍历检测（Doppler 维可环形，距离维仍跳过 R）
    ...
    const int r_min = cfg.cfar_doppler_circular ? 0 : R;
    const int r_max = cfg.cfar_doppler_circular ? H - 1 : H - 1 - R;
    ...
    for (int r = r_min; r <= r_max; ++r)
    {
        for (int c = c_min; c <= c_max; ++c)
        {
            ...
            if (ty == "CA")
            {
                const double sum_full = doppler_rect_sum(r0, c0, r1, c1);
                const double sum_guard = doppler_rect_sum(rg0, cg0, rg1, cg1);
            ...
            { // "GO"
                ...
                if (lc0 <= lc1)
                {
                    const double s = doppler_rect_sum(r0, lc0, r1, lc1);
                ...
                if (rc0 <= rc1)
                {
                    const double s = doppler_rect_sum(r0, rc0, r1, rc1);
                ...
                if (tr0 <= tr1)
                {
                    const double s = doppler_rect_sum(tr0, c0, tr1, c1);
                ...
                if (br0 <= br1)
                {
                    const double s = doppler_rect_sum(br0, c0, br1, c1);
            ...
            const bool excluded = cfg.cfar_exclude_row_start >= 0 &&
                                  r >= cfg.cfar_exclude_row_start &&
                                  r <= cfg.cfar_exclude_row_end;
            if (CUT > thr && !excluded)
            {
                mydata[idx] = CUT;
                prow.push_back(r);
