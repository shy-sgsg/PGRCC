                                           ...
                                           std::vector<cd> &a2)
{
    ...
    if (skip > 0)
    {
        ...
        if (s >= Na)
            return true;
        const std::size_t rows_copy = Na - s;
        const std::size_t src_row0 = s;
                                   ...
                                   std::vector<std::complex<double>> &out2)
{
    ...
    if (window_type != gmti::azimuth_window::None) {
        for (std::size_t row = 0; row < Na; ++row) {
            const double w = gmti::azimuth_window::normalizedValue(
                window_type, row, Na, cfg.azimuth_fft_window_normalize);
            const std::size_t off = row * Nr;
...
bool GMTIProcessor::initcuFFTPlans(const Config &cfg) {
    ...
    if (cfg.INFO_Type && !cfg.isPC && cfg.new_protocol_gpu_preprocess) {
        // The packed PRT staging region reuses a1/a2/t1/t2 (four adjacent
        // workspace slots).  Include per-PRT headers when sizing those slots;
        // sizing only for four-channel IQ makes the packed buffer overflow by
        // exactly header_bytes * pulse_count and forces a CPU fallback.
        const std::size_t packed_bytes =
            gmti::new_protocol::packetBytes(
                static_cast<std::size_t>(cfg.pulse_len),
                static_cast<std::size_t>(std::max(2, cfg.new_protocol_channel_count)),
                cfg.iq_data_type.empty() ? "float32" : cfg.iq_data_type) * Na;
    ...
    if (d_workspace == nullptr || d_workspace_bytes < needed_bytes) {
        ...
        err = cudaMalloc(&d_phi_fit_, Nr * sizeof(float));
        if (err != cudaSuccess) {
            cudaFree(d_workspace);
            d_workspace = nullptr;
            d_workspace_bytes = 0;
            ...
            ERR("GPU p38 phase-buffer allocation failed: "
    ...
    // DBS幅度是实数导出结果，必须拥有独立的生命周期和存储空间。
    // 它不能落入 d1/d2/a1/a2/t1/t2/csi 中的任何一个复数缓冲，否则
    // exportDbsCacheAfterRecenter() 返回后 CSI 第二遍会读到被破坏的通道数据。
    const size_t amplitude_elements = Na * Nr;
    if (gpu_ptrs_.dbs_amplitude == nullptr ||
        dbs_amplitude_elements_ < amplitude_elements) {
        if (gpu_ptrs_.dbs_amplitude != nullptr) {
            cudaFree(gpu_ptrs_.dbs_amplitude);
            gpu_ptrs_.dbs_amplitude = nullptr;
            dbs_amplitude_elements_ = 0;
        }
        const cudaError_t amplitude_err = cudaMalloc(
            &gpu_ptrs_.dbs_amplitude,
            amplitude_elements * sizeof(float));
        if (amplitude_err != cudaSuccess) {
            ERR("GPU DBS amplitude-buffer allocation failed: "
                << cudaGetErrorString(amplitude_err));
            gpu_ptrs_.dbs_amplitude = nullptr;
            return false;
        }
        dbs_amplitude_elements_ = amplitude_elements;
        DBG("CUDA DBS amplitude buffer allocated: "
            << (amplitude_elements * sizeof(float)) / (1024 * 1024)
            << " MB");
    }
