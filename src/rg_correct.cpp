                            ...
                            double coeff_out[3])                    // 最终系数 a,b,c
{
    ...
    // The range-phase correction is part of the production signal path, so it
    // must be bit-for-bit reproducible for the same echo.  The old
    // random_device seed (and sampling with replacement) changed phi_fit,
    // p38 and the final detection count between identical runs.  Use a fixed,
    // input-size-mixed seed and draw unique indices in every trial.
    std::mt19937 rng(0x47524d54u ^ static_cast<unsigned int>(len_t));
    std::vector<int> order(static_cast<size_t>(len_t));
    ...
    for (int it = 0; it < final_count; ++it) {
        ...
        for (int k = 0; k < use_num; ++k) {
            const int idx = order[static_cast<size_t>(k)];
            xs[k] = static_cast<double>(xs_1based[idx]);
            ys[k] = ys_phase[idx];
        }
    ...
    std::vector<int>&    phi_diss_range)
{
    ...
    // Use the validated/clamped support.  The configured support may come
    // from a legacy 4096-bin profile while the current workbook-derived
    // matrix has 4032 bins; indexing with the raw XML bounds corrupts both
    // this host vector and the device range-sum buffer.
    for (int c = rg_st, k = 0; c <= rg_ed; ++c, ++k) {
        // 直接对 GPU 传回的累加结果求相位
