...
static bool parse_gmti_bin_index(const char *name, int &index)
{
    index = 0;
    const std::string value(name);
    constexpr size_t kPrefixLength = 4U;
    constexpr size_t kSuffixLength = 4U;
    if (value.size() <= kPrefixLength + kSuffixLength ||
        value.compare(0, kPrefixLength, "GMTI") != 0 ||
        value.compare(value.size() - kSuffixLength, kSuffixLength, ".bin") != 0) {
        return false;
    }

    const size_t digits_end = value.size() - kSuffixLength;
    int parsed = 0;
    for (size_t pos = kPrefixLength; pos < digits_end; ++pos) {
...
static int resolveCurrentResultId(const Config& cfg)
{
    ...
    while (dirent* ent = readdir(dir)) {
        int id = -1;
        if (parse_gmti_bin_index(ent->d_name, id) && id > latest) {
            latest = id;
        }
    }
    closedir(dir);
...
{
    const std::string dir = cfg.track_debug_dir.empty()
        ? cfg.result_add : cfg.track_debug_dir;
    const std::string path = dir + "/track_points.csv";
...
std::vector<GMTIDetection> trackModuleOnline(const Config& cfg, TrackManager* manager)
{
    ...
    // TrackManager materializes coordinates according to
    // track_output_state_source.  In measurement mode this deliberately keeps
    // the exact current-detection lon/lat; prediction/filtered modes project
    // their selected EN snapshot before returning it.
