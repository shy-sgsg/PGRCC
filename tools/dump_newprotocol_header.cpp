...
void printHeader(const std::string &path, std::size_t packet_bytes, std::size_t pulse_idx)
{
    ...
    std::cout << pulse_idx << ","
              << gmti::new_protocol::loadU32LE(
                     hdr.data() + gmti::new_protocol::kOffPrtCounter) << ","
              << std::setprecision(12)
              << s.utc << ","
              << s.lat_deg << ","
              << s.lon_deg << ","
    ...
    try {
        ...
        std::cout << "# packet_bytes=" << packet_bytes
                  ...
                  << ((packet_bytes - gmti::new_protocol::kHeaderBytes) /
                      sample_bytes)
                  << " total_prt=" << total << "\n";
        std::cout << "pulse_idx,prt_counter,utc,lat,lon,h,vn,ve,vd,heading_deg,"
                     "servo_azimuth_deg,servo_elevation_deg,scan_center_azimuth_deg,"
                     "scan_center_elevation_deg,scan_speed_azimuth_deg_s,"
                     "scan_extent_azimuth_deg\n";
        std::vector<std::size_t> indices;
        indices.push_back(0U);
        if (total > 129U) indices.push_back(129U);
        if (total > 130U) indices.push_back(130U);
        indices.push_back(total / 2U);
        if (total > 130U) indices.push_back(total - 130U);
        indices.push_back(total - 1U);
        std::sort(indices.begin(), indices.end());
        indices.erase(std::unique(indices.begin(), indices.end()), indices.end());
        for (std::size_t i = 0; i < indices.size(); ++i) {
            printHeader(path, packet_bytes, indices[i]);
        }
