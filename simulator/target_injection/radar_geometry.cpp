...
GeometrySample evaluateGeometry(const RadarConfig &radar,
                                ...
                                int pulse_id)
{
    return evaluateGeometryAtTimeAndAngle(
        radar, global, target,
        pulseTimeSec(radar, period_id, beam_id, pulse_id),
        radar.scan_min_deg + radar.scan_step_deg * static_cast<double>(beam_id));
}

GeometrySample evaluateGeometryAtTimeAndAngle(const RadarConfig &radar,
                                              const TargetGlobalConfig &global,
                                              const TargetConfig &target,
                                              double time_sec,
                                              double servo_azimuth_deg)
{
    GeometrySample g;
    g.time_sec = time_sec;
    g.theta_cmd_deg = servo_azimuth_deg;
