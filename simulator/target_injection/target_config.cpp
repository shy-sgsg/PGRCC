...
std::vector<std::string> arrayObjects(const std::string &txt,
                                      const std::string &key)
{
    std::vector<std::string> objects;
    const std::string needle = "\"" + key + "\"";
    const size_t k = txt.find(needle);
    if (k == std::string::npos) return objects;
    const size_t open_arr = txt.find('[', k);
    if (open_arr == std::string::npos) return objects;
    int array_depth = 1;
    int object_depth = 0;
    size_t object_start = std::string::npos;
    bool in_string = false;
    bool escaped = false;
    for (size_t i = open_arr + 1; i < txt.size() && array_depth > 0; ++i) {
        const char ch = txt[i];
        if (in_string) {
        ...
        if (ch == '"') {
            ...
            if (--object_depth == 0 && object_start != std::string::npos) {
                objects.push_back(txt.substr(object_start, i - object_start + 1));
                                ...
                                TargetConfig &target)
{
    ...
    const gmti::sim_geometry::LookVectorEN look =
        gmti::sim_geometry::makeAlgorithmLookVectorEN(ref_enu_velocity.ve,
                                                      ...
                                                      target.azimuth_deg,
                                                      global.geometry);
    const double tangential_speed_mps = target.cross_velocity_mps;
    // Positive cross velocity is defined as moving to the left of the LOS/beam look direction.
    // This keeps radial/cross decomposition consistent with the left/right look convention
    // used by makeAlgorithmLookVectorEN().
    ...
    const gmti::sim_geometry::ENUVelocity target_enu_velocity(
        target.radial_velocity_mps * look.east + tangential_speed_mps * cross_left.east,
        target.radial_velocity_mps * look.north + tangential_speed_mps * cross_left.north,
        0.0);
...
bool loadRadarConfig(const std::string &xml_path, RadarConfig &cfg, std::string &err)
{
    ...
    const std::string scan_mode = textOf(param, "scan_mode");
    if (scan_mode == "mechanical") {
        TiXmlElement *scan = param->FirstChildElement("mechanical_scan");
        if (scan) {
            cfg.phase_center_rotation_enable = toInt(
                textOf(scan, "phase_center_rotation_enable"),
                cfg.phase_center_rotation_enable ? 1 : 0) != 0;
            cfg.phase_center_mount_angle_deg = toDouble(
                textOf(scan, "phase_center_mount_angle_deg"),
                cfg.phase_center_mount_angle_deg);
            cfg.phase_center_rotation_sign = toInt(
                textOf(scan, "phase_center_rotation_sign"),
                cfg.phase_center_rotation_sign);
        }
    }
    if (cfg.new_protocol_channel_count > 2) {
        std::vector<Vec3> offsets;
        offsets.reserve(static_cast<std::size_t>(cfg.new_protocol_channel_count));
        bool complete = true;
        for (int ch = 1; ch <= cfg.new_protocol_channel_count; ++ch) {
            const std::string prefix = "four_channel_ch" + std::to_string(ch);
            const std::string x_text = textOf(param, (prefix + "_x_m").c_str());
            const std::string y_text = textOf(param, (prefix + "_y_m").c_str());
            const std::string z_text = textOf(param, (prefix + "_z_m").c_str());
            if (x_text.empty() || y_text.empty() || z_text.empty()) {
                complete = false;
                break;
            }
            offsets.push_back(Vec3(
                toDouble(x_text, 0.0), toDouble(y_text, 0.0),
                toDouble(z_text, 0.0)));
        }
        if (complete) cfg.channel_offsets_local_m = offsets;
    }
    return true;
}
                      ...
                      std::string &err)
{
    ...
    global.lfm_time_reference = jsonString(
    ...
    if (global.lfm_time_reference != "center" &&
        global.lfm_time_reference != "start" &&
        global.lfm_time_reference != "legacy_start") {
        err = "global.lfm_time_reference must be center or legacy_start";
        return false;
    }
    resolveTargetInitialState(radar, global, target);
    return true;
}
...
bool loadTargetConfigs(const std::string &json_path,
                       ...
                       std::string &err)
{
    // Reuse the compatibility loader for all global/simulation-geometry
    // fields, then parse every target object with identical per-target rules.
    TargetConfig compatibility_first;
    if (!loadTargetConfig(json_path, radar, global, compatibility_first, err)) {
        return false;
    }
    const std::string txt = readTextFile(json_path);
    const std::vector<std::string> objects = arrayObjects(txt, "targets");
    if (objects.empty()) {
        err = "targets.json must contain a non-empty targets array";
        return false;
    }
    ...
    for (size_t i = 0; i < objects.size(); ++i) {
        const std::string &t = objects[i];
        TargetConfig target;
        target.id = jsonInt(t, "id", static_cast<int>(i + 1));
        target.name = jsonString(t, "name", "target_" + std::to_string(i + 1));
        target.motion_model = jsonString(t, "motion_model", target.motion_model);
        target.init_mode = jsonString(t, "init_mode", target.init_mode);
        target.enabled = jsonBool(t, "enabled", target.enabled);
        target.start_period = jsonInt(t, "start_period", target.start_period);
        ...
        target.azimuth_deg = jsonDouble(t, "azimuth_deg", target.azimuth_deg);
        target.height_m = jsonDouble(t, "height_m", target.height_m);
        target.radial_velocity_mps = jsonDouble(
            t, "radial_velocity_mps", target.radial_velocity_mps);
        target.cross_velocity_mps = jsonDouble(
            t, "cross_velocity_mps", target.cross_velocity_mps);
        target.target_snr_db = jsonDouble(t, "target_snr_db", target.target_snr_db);
        target.visibility_mode = jsonString(t, "visibility_mode", target.visibility_mode);
        target.single_beam_only = jsonBool(t, "single_beam_only", target.single_beam_only);
        target.visible_beam_span = jsonInt(t, "visible_beam_span", target.visible_beam_span);

        if (!ids.insert(target.id).second) {
            err = "duplicate target id: " + std::to_string(target.id);
            return false;
        }
        if (target.name.empty() || !names.insert(target.name).second) {
            err = "duplicate or empty target name: " + target.name;
            return false;
        }
        if (target.end_period < target.start_period) {
            err = "target " + target.name + " has end_period < start_period";
            return false;
        }
        resolveTargetInitialState(radar, global, target);
