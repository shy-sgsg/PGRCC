...
bool TruthWriter::open(const std::string &truth_dir, std::string &err)
{
    ...
    pulse_ << "period_id,beam_id_0based,beam_id_1based,pulse_id,time_sec,target_id,target_name,"
           ...
           << "phi_total_truth_rad,phi_total_unwrapped_truth_rad,phase_ambiguity_order_truth,"
           << "phi_static_truth_rad,phi_motion_truth_rad,"
           << "path_ch1_m,path_ch2_m,path_ch3_m,path_ch4_m,"
           << "phase_ch1_rad,phase_ch2_rad,phase_ch3_rad,phase_ch4_rad,"
           << "phase_13_rad,phase_24_rad\n";
    summary_ << "case_id,target_id,period_id,beam_id,utc_mid,"
             ...
             << "visible,snr_db,af_motion_truth_hz,af_geometry_truth_hz,af_total_truth_hz,row_truth,"
             << "phi_total_truth_rad,phi_total_unwrapped_truth_rad,phase_ambiguity_order_truth\n";
...
void TruthWriter::writePulse(const PulseTruth &t)
{
    ...
    pulse_ << std::setprecision(12)
           ...
           << t.phi_total_unwrapped_truth_rad << ","
           << t.phase_ambiguity_order_truth << ","
           << t.phi_static_truth_rad << ","
           << t.phi_motion_truth_rad;
    const double nan = std::numeric_limits<double>::quiet_NaN();
    for (std::size_t ch = 0; ch < 4; ++ch) {
        pulse_ << "," << (ch < t.channel_path_length_m.size()
                              ? t.channel_path_length_m[ch] : nan);
    }
    for (std::size_t ch = 0; ch < 4; ++ch) {
        pulse_ << "," << (ch < t.channel_phase_rad.size()
                              ? t.channel_phase_rad[ch] : nan);
    }
    pulse_ << "," << (t.channel_phase_rad.size() >= 3
                          ? t.channel_phase_rad[0] - t.channel_phase_rad[2] : nan)
           << "," << (t.channel_phase_rad.size() >= 4
                          ? t.channel_phase_rad[1] - t.channel_phase_rad[3] : nan)
           << "\n";
    ...
    const int desired_mid_pulse = t.ref_pulse_idx >= 0 ? t.ref_pulse_idx : 0;
    const int mid_pulse_distance = std::abs(t.pulse_id - desired_mid_pulse);
    if (!a.has_mid_truth || mid_pulse_distance < a.mid_pulse_distance) {
        a.has_mid_truth = true;
        a.mid_pulse_distance = mid_pulse_distance;
        a.mid_truth = t;
    }
    if (t.has_ref_geometry) {
        ...
        a.phi_total_unwrapped_truth_rad = t.phi_total_unwrapped_truth_rad;
        a.phase_ambiguity_order_truth = t.phase_ambiguity_order_truth;
...
void TruthWriter::writeSummary()
{
    for (std::map<std::string, BeamSummaryAccumulator>::const_iterator it = acc_.begin();
        ...
        const PulseTruth *mid = a.has_mid_truth ? &a.mid_truth : nullptr;
        const GeometrySample *g = mid ? &mid->geom : nullptr;
        summary_ << std::setprecision(12)
                 << case_id_ << ","
                 << a.target_name << ","
                 << a.period_id << ","
                 << (a.beam_id + 1) << ","
                 << (g ? g->time_sec : (a.has_ref_geometry ? a.ref_time_s : 0.0)) << ","
                 << (mid ? mid->target_e : (a.has_ref_geometry ? a.ref_target_e : 0.0)) << ","
                 << (mid ? mid->target_n : (a.has_ref_geometry ? a.ref_target_n : 0.0)) << ","
                 << (mid ? mid->target_lat : (a.has_ref_geometry ? a.ref_target_lat : 0.0)) << ","
                 << (mid ? mid->target_lon : (a.has_ref_geometry ? a.ref_target_lon : 0.0)) << ","
                 << (g ? g->range_m : a.sum_range_m / denom) << ","
                 << (mid ? mid->expected_range_bin : -1) << ","
                 << (mid ? mid->expected_range_bin : -1) << ","
                 << (g ? g->theta_cmd_deg : a.sum_theta_cmd_deg / denom) << ","
                 << (g ? g->target_azimuth_deg : a.sum_target_azimuth_deg / denom) << ","
                 << (mid ? mid->target_ve_mps : std::numeric_limits<double>::quiet_NaN()) << ","
                 << (mid ? mid->target_vn_mps : std::numeric_limits<double>::quiet_NaN()) << ","
                 << (mid ? mid->target_vr_self_mps : std::numeric_limits<double>::quiet_NaN()) << ","
                 << (mid ? mid->target_vt_self_mps : std::numeric_limits<double>::quiet_NaN()) << ","
                 << (a.visible_pulse_count > 0 ? 1 : 0) << ","
                 << (mid ? mid->snr_db : std::numeric_limits<double>::quiet_NaN()) << ","
                 << (mid ? mid->af_motion_truth_hz : std::numeric_limits<double>::quiet_NaN()) << ","
                 << (mid ? mid->af_geometry_truth_hz : std::numeric_limits<double>::quiet_NaN()) << ","
                 << (mid ? mid->af_total_truth_hz : std::numeric_limits<double>::quiet_NaN()) << ","
                 << (mid ? mid->row_truth : -1) << ","
                 << (mid ? mid->phi_total_truth_rad : std::numeric_limits<double>::quiet_NaN()) << ","
                 << (mid ? mid->phi_total_unwrapped_truth_rad : std::numeric_limits<double>::quiet_NaN()) << ","
                 << (mid ? mid->phase_ambiguity_order_truth : 0)
                 << "\n";
    }
}
                                ...
                                std::string &err)
{
    ...
    out << "- lfm_time_reference: " << cfg.global.lfm_time_reference << "\n";
    ...
    out << "- LFM 时间参考为 `" << cfg.global.lfm_time_reference
        << "`；`center` 使用 `-Tr/2 <= t_fast - tau_rel < Tr/2`，"
           "`legacy_start` 才使用旧的 `0 <= t_fast - tau_rel < Tr`。"
           "脉压后期望位置按 `round(range_sample_float - range_crop_start)` 记录在 truth。\n";
    out << "- 平台模型默认为 `ideal_platform`；真实 POS/导航误差不在该模型内。\n";
