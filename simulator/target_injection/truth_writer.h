...
struct BeamSummaryAccumulator {
    ...
    // The target reference geometry is anchored once when the scenario is
    // expanded.  It is therefore unsuitable for a later scan/period summary.
    // Keep the actual pulse nearest ref_pulse_idx for this period/beam so that
    // truth_targets_by_beam.csv follows a moving target in time and range.
    bool has_mid_truth = false;
    int mid_pulse_distance = std::numeric_limits<int>::max();
    PulseTruth mid_truth;
