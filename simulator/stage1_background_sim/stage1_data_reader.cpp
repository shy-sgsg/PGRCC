                  ...
                  std::string &err)
{
    ...
    for (int p = 0; p < cfg.pulse_num; ++p) {
        ...
        if (header.size() >= 44U) {
            ...
            // Some Mission068 old blocks wrap the fractional-second field
            // from 0.999... to 0.000... without incrementing <ss>.  Unwrap
            // only an unambiguous near-one-second backward jump so the
            // converted protocol UTC remains physically ordered.
            if (p > 0) {
                const double previous = block.utc[static_cast<size_t>(p - 1)];
                while (utc + 0.5 < previous) utc += 1.0;
            }
            block.utc[static_cast<size_t>(p)] = utc;
        }
