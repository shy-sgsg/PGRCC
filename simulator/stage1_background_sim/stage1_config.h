...
struct Stage1NewSystemConfig {
    // Align odd 2-degree new beams with the old -25,-23,...,25-degree
    // acquisition.  This produces 60 beams from -59 to 59 degrees.
    double scan_min_deg = -59.0;
    double scan_max_deg = 59.0;
...
struct Stage1RunOptions {
    ...
    int beam_start = 0;
    int beam_count = 0;
    // Semicolon-separated RTIPOS files.  Empty means <Plane_POS> from the
    // old XML.  This keeps a multi-period conversion tied to the matching
    // inertial-data span instead of silently clamping to one POS segment.
