...
double mirrorToOldAngleRange(double theta)
{
    double ref = theta;
    // Mirror around the two measured old-aperture edges.  Unlike modulo
    // wrapping, 25 -> 27 degrees becomes 25 -> 23 degrees rather than an
    // artificial jump from 25 to -25.  With the odd-angle Stage1 grid, the
    // entire measured -25..25 degree region maps one-to-one to raw beams.
    while (ref > 25.0 || ref < -25.0) {
        if (ref > 25.0) ref = 50.0 - ref;
        if (ref < -25.0) ref = -50.0 - ref;
    }
    return ref;
}
...
bool buildBeamMap(const Stage1NewSystemConfig &cfg, std::vector<BeamMapEntry> &map, std::string &err)
{
    ...
    for (size_t k = 0; k < new_angles.size(); ++k) {
        ...
        const double ref = mirrorToOldAngleRange(theta);
