...
public:
    ...
    bool read(void* out_buf, size_t& io_len) {
        ...
        size_t available = (current_write - current_read + ctrl_->capacity) &
            (ctrl_->capacity - 1);
...
public:
    ...
    // Receiver-side ABI sanity checks need the mapped object size.  This does
    // not alter the shared-memory layout or producer/consumer semantics.
    size_t mapped_size() const { return total_size_; }
...
private:
    ...
    void attach_receiver() {
        ...
        if (st.st_size < static_cast<off_t>(sizeof(SharedControl)))
            throw std::runtime_error("shared-memory object is smaller than RingBuffer control header");
